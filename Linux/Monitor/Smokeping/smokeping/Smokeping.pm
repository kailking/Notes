...
sub sendmail ($$$){
	...
	$smtp->datasend("Content-Type:text/plain; charset=UTF-8\n");
        ...
        $smtp->datasend("To: $to\n");
        $smtp->datasend("Date: $rfc2822stamp\n");
        $smtp->datasend("Subject:=?UTF-8?B?".encode_base64($charlie_subject, '')."?=\n\n");
        $smtp->datasend($body);
        $smtp->dataend();
        $smtp->quit;
...
sub target_menu($$$$;$){
	...
	our $charlie_alias;
		...
		$charlie_alias = $tree->{$key}{__tree_link}{alias};
...
sub check_alerts {
    ...
    if ( $tree->{alerts} ) {
		...
		our $charlie_alias;
                my $charlie_prop = $line;
                $charlie_prop =~ s|([^\.]*\.)*||g;
                $charlie_prop = "$slave -> $charlie_prop" if $slave;
                do_log("[SmokeAlert] $_ $charlie_prop");
                my $loss = "--丢包: ".join ", ",map {defined $_ ? (/^\d/ ? sprintf "%.0f%%", $_ :$_):"U" } @{$x->{loss}};
                my $rtt = "--延迟: ".join ", ",map {defined $_ ? (/^\d/ ? sprintf "%.0fms", $_*1000 :$_):"U" } @{$x->{rtt}}; 
                        my $time = time;
...
DOC
			our $charlie_subject = "$_ $charlie_prop";
                    ...
                    our $rfc2822stamp =  POSIX::strftime("%a, %e %b %Y %H:%M:%S %z", @stamp);
                                my $to = join ",",@to;
...
sub update_rrds($$$$$$) {
    ...
    foreach my $prop (keys %{$tree}) {
		...
		our $charlie_alias = $tree->{title};
		#our $charlie_alias = $tree->{alias};
...
DOC
                   _vars       => [ qw(probe menu title remark alerts slaves menuextra parents alias) ],
